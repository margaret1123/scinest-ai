...
export default function Page() {
  ...
  return <div className={styles.page}>
    ...
    <main>
      <section className={styles.hero}><div><p className={styles.eyebrow}>无需换工具，无需重画整张图</p><h1>把研究内容变成一张<em>还能继续修改的科研图。</em></h1><p className={styles.lead}>先理解实体、过程、阶段和关系，再生成机制图、路线图、实验流程图或图形摘要。标签和结构发生变化时，不必推翻整张图。</p><div className={styles.points}>{["可编辑标签","关系可调整","局部修改","多种图型","可复用到 PPT"].map(x=><span key={x}>✓ {x}</span>)}</div><div className={styles.actions}><a className={styles.cta} href={downloadUrl}>生成我的科研图 ↗</a><a className={styles.secondary} href="#workflow">查看流程</a></div></div><figure className={styles.proof}><img src="/scinest/figures-ui-en.webp" alt="SciNest 科研图生成与编辑工作区" width="1280" height="800" /></figure></section>
      <section className={`${styles.section} ${styles.dark}`} id="difference"><div><p className={styles.kicker}>文字可编辑，元素可移动，局部可重绘</p><h2>标签、形状和区域生成后保持独立可改——只动需要改的部分。</h2><div className={styles.grid}><article className={styles.card}><h3>标签单独修改</h3><p>文字与标注尽量保持可编辑，减少拼写错误和术语变化带来的返工。</p></article><article className={styles.card}><h3>关系可以调整</h3><p>箭头方向、阶段顺序和模块关系可以继续修正。</p></article><article className={styles.card}><h3>保留已确认区域</h3><p>修改局部时，不应丢掉已经认可的其他部分。</p></article></div></div></section>
      ...
      <section className={styles.final}><h2>一张科研图，可编辑图层，无需从头来。</h2><p>上传材料，生成图表，继续修改和复用。免费开始。</p><a className={styles.cta} href={downloadUrl}>获取 SciNest Free ↗</a></section>
